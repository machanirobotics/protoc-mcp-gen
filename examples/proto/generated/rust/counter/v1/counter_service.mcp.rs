...
const COUNTER_SERVICE__COUNT_OUTPUT_SCHEMA_JSON: &str = r##"{"properties":{"count":{"type":"integer"}},"required":[],"type":"object"}"##;
...
pub trait CounterServiceMcpServer: Send + Sync + 'static {
    ...
    async fn count(&self, args: Value, progress: McpProgressSink) -> std::result::Result<Value, McpError>;
}
...
impl<T: CounterServiceMcpServer> CounterServiceMcpHandler<T> {
    ...
    pub fn tools() -> Vec<Tool> {
        ...
        vec![
            make_tool_with_app_meta("counter_service-count_v1", "Counts from 0 up to the given number. Sends progress updates as it counts. Use with progressToken in _meta for progress notifications.", COUNTER_SERVICE__COUNT_SCHEMA_JSON, COUNTER_SERVICE__COUNT_OUTPUT_SCHEMA_JSON, Value::Null, "", Value::Null, &app_uri),
        ]
    }
